...
CONFIRM_FLIP = True
CONFIRM_WINDOW_SECS = 5          # (changed from 60 to 5)
...
# ───────────── POSITION VALIDATION ─────────
async def get_position_size(cli: AsyncClient, symbol: str, side: str) -> float | None:
    """Get current position size from Binance. Returns None on failure (don't assume flat)."""
    try:
        positions = await cli.futures_position_information(symbol=symbol)
        for pos in positions:
            if pos.get("positionSide") == side:
...
async def get_all_position_sizes(cli: AsyncClient) -> dict[str, dict[str, float]] | None:
    """
    Returns a nested dict of sizes for ALL symbols in one request:
      { "ADAUSDT": {"LONG": 1.0, "SHORT": 0.0}, ... }
    Returns None on failure.
    """
    try:
        positions = await cli.futures_position_information()
        out: dict[str, dict[str, float]] = {}
        for p in positions:
            sym = p.get("symbol")
            side = p.get("positionSide")
            if sym is None or side not in ("LONG", "SHORT"):
                continue
            size = abs(float(p.get("positionAmt", 0.0)))
            out.setdefault(sym, {"LONG": 0.0, "SHORT": 0.0})
            out[sym][side] = size
        return out
    except Exception as e:
        logging.error(f"get_all_position_sizes failed: {e}")
        return None
...
async def safe_order_execution(cli: AsyncClient, order_params: dict, symbol: str, action: str) -> bool:
    """Execute order with duplicate prevention and validation (no clientOrderId injection)."""
    try:
        ...
        if action.startswith("CLOSE") or "EXIT" in action:
            ...
            # Only enforce when we actually know it
            if current_pos is not None and current_pos < required_qty * 0.99:
...
# ─ NEW: keep state in sync if something flattened positions (batched fetch)
async def position_sync_scheduler(cli: AsyncClient):
    while True:
        try:
            snap = await get_all_position_sizes(cli)
            if snap is None:
                # Rate-limited or error: don't guess, just try later
                await asyncio.sleep(3)
                continue

            for symbol in SYMBOLS:
                st = state[symbol]
                sizes = snap.get(symbol, {"LONG": 0.0, "SHORT": 0.0})
                long_sz  = sizes.get("LONG", 0.0)
                short_sz = sizes.get("SHORT", 0.0)
