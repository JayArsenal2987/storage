    ...
    while True:
        ...
        for symbol in SYMBOLS:
            ...
            if candle['is_green'] and candle['has_upward_momentum'] and not has_long:
                logging.info(f"→ Signal: LONG (green candle + close > prev close)")
                ...
                logging.info(f"→ GREEN candle but NO upward momentum (close <= prev close), skipping LONG")
            ...
            if candle['is_red'] and candle['has_downward_momentum'] and not has_short:
                logging.info(f"→ Signal: SHORT (red candle + close < prev close)")
                ...
                logging.info(f"→ RED candle but NO downward momentum (close >= prev close), skipping SHORT")
...
async def init_bot(client: AsyncClient):
    ...
    logging.info(f"📊 Strategy: 1-minute candle based")
    logging.info(f"📊 Entry: Last closed candle (GREEN=LONG, RED=SHORT)")
...
async def main():
    ...
    try:
        ...
        logging.info("🚀 Bot started - Trading based on 1-minute closed candles with 0.5% trailing stops")
...
if __name__ == "__main__":
    ...
    print(f"  - GREEN candle (close > open) → LONG")
    print(f"  - RED candle (close < open) → SHORT")
