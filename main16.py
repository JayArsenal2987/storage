...
# ---- SYMBOLS ----
# NOTE: Only change requested by you: add these symbols with the exact sizes.
SYMBOLS = {
    "BTCUSDT": 0.001,
    "ETHUSDT": 0.01,
    "BNBUSDT": 0.03,
...
WMA_EQUAL_TOL   = float(os.getenv("WMA_EQUAL_TOL", "0.002"))            # ±0.2% ~ FLAT band

# ---- Bounce (24h latest extreme) ----
BOUNCE_ENABLED       = True
# kept for visibility; bounce uses 24h only as requested
BOUNCE_WINDOWS_SEC   = [24*3600]
BOUNCE_THRESHOLD_PCT = float(os.getenv("BOUNCE_THRESHOLD_PCT", "2.0")) / 100.0  # 2%
...
async def verify_position_after_trade(cli: AsyncClient, symbol: str, expected_long: float, expected_short: float) -> bool:
    ...
    try:
        ...
        # Use dynamic tolerance based on position size (0.1% minimum, 0.1 ADA minimum)
...
# ========================= SYMBOL TARGET SIZE ==================
# Minimal helper so each symbol uses its specified target size.
...
def bounce_signal(st: dict) -> Optional[str]:
    """
    IMPROVED: Bounce uses distance-based priority when both conditions are met.
    Choose the bounce signal based on which bounce TRIGGER price is closer to.
    """
...
async def set_target_position(cli: AsyncClient, symbol: str, target_signal: str):
    ...
    async with st["order_lock"]:
        ...
        try:
            ...
            # Minimal change: use per-symbol target size you requested.
            ...
            # IMPROVEMENT: Verify final positions match expectations
            if ok_all:
...
async def simplified_trading_loop(cli: AsyncClient):
    ...
    while True:
        ...
        for sym in SYMBOLS:
            ...
            # Activation: allow bounce OR directional WMA to start the bot
            ...
            if target != current and current in ("LONG","SHORT") and st.get("last_position_entry_ts",0) > 0:
                if now - st["last_position_entry_ts"] < MIN_HOLD_SEC:  # enforce min hold
                    continue
                if not _callback_retrace_hit(st, price, current):      # enforce callback retrace
                    continue
            ...
            # --- Staggered Execution (per your request) ---
