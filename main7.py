...
# ───────────── Dynamic trailing mapping (PnL% ROE → trailing distance %) ─────────────
...
state = {
    s: {
        ...
        # NEW: record the special client IDs used for trailing orders
...
def callback_from_tiers(pnl_pct_roe: float) -> Optional[float]:
    """
    Map ROE% to a trailing callback rate using dynamic rules.
    Returns None if ROE is between the first loss/profit thresholds (i.e., keep current trailing).
    """
    if not math.isfinite(pnl_pct_roe):
        return None

    # Profits: choose the tightest rule whose threshold is met (highest threshold first).
    for th, cb in sorted(DYNAMIC_TRAIL_RULES, key=lambda x: x[0], reverse=True):
        if pnl_pct_roe >= th:
            return cb

    # Losses: tighten as loss deepens (most negative first).
    for th, cb in sorted(DYNAMIC_TRAIL_RULES_LOSS, key=lambda x: x[0]):
        if pnl_pct_roe <= th:
            return cb

    # Between thresholds → don't change/create trailing
    return None
