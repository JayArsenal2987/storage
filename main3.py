...
CONFIRM_FLIP = True
CONFIRM_WINDOW_SECS = 5          # already reduced per your request
CONFIRM_LOOKBACK_MIN = 60        # minutes of 1m candles to derive swing levels (from websocket)
CONFIRM_BREAK_PCT = 0.0          # extra break beyond swing level
...
# ─ NEW: exchange-native trailing stop helpers (no open-orders listing)
async def cancel_trailing_stop(cli: AsyncClient, symbol: str, side: str):
    """Cancel existing trailing stop for symbol+side using known orderId; no REST listing."""
    try:
        st = state[symbol]
        oid = st.get("server_trailing_order_id")
        if not oid:
            return
        await cli.futures_cancel_order(symbol=symbol, orderId=oid)
        logging.info(f"{symbol} {side} canceled existing trailing stop {oid}")
...
async def place_trailing_stop(cli: AsyncClient, symbol: str, side: str, callback_rate: float = None):
    ...
    """
    try:
        qty = SYMBOLS[symbol]  # avoid REST; use configured qty
        if qty <= 0:
            return False

        # If we have a known trailing order id, cancel it first (no listing)
...
state = {
    s: {
        ...
        "confirm_until": 0.0,
        "confirm_side": None,
        "confirm_swing_level": None,

        # De-dup
        "last_action_ts": {},
        "last_ts_filled_id": None,

        # Position sizes kept from ACCOUNT_UPDATE (no REST)
        "pos_size_long": 0.0,
        "pos_size_short": 0.0,
    }
    for s in SYMBOLS
...
# In-memory 1m candles from websocket: {symbol: deque[[openTime, o,h,l,c,v], ...]}
candle_store = {s: deque(maxlen=HISTORICAL_PERIOD_MINUTES) for s in SYMBOLS}

# ───────────── POSITION/SIZE (no REST) ─────────
def _get_cached_position_size(symbol: str, side: str) -> float:
    st = state[symbol]
    if side == "LONG":
        return float(st.get("pos_size_long") or 0.0)
    else:
        return float(st.get("pos_size_short") or 0.0)
...
async def safe_order_execution(cli: AsyncClient, order_params: dict, symbol: str, action: str) -> bool:
    try:
        ...
        # Size validation for closes using cached sizes
        if action.startswith("CLOSE") or "EXIT" in action:
            side = "LONG" if "LONG" in action else "SHORT"
            current_pos = _get_cached_position_size(symbol, side)
            required_qty = order_params['quantity']
            if current_pos < required_qty * 0.99:
                logging.warning(f"{symbol} {action}: Insufficient cached size {current_pos} < {required_qty}")
                return False
...
# ───────────── ROLLING ANALYSIS (Initial Direction Only, websocket candles) ─────────
async def perform_rolling_analysis_ws(symbol: str, current_price: float):
    """Use websocket-fed 1m candles from candle_store (no REST)."""
    try:
        candles = list(candle_store[symbol])
        if not candles:
            return None
        ...
        logging.error(f"{symbol} Rolling analysis (ws) failed: {e}")
        return None
...
async def handle_trailing_stops(cli: AsyncClient, sym: str, price: float):
    ...
    if target_cb is not None:
        ...
        if target_dec < st["local_trail_pct"]:
            ...
            logging.info(f"{sym} {side} local trailing tightened: {prev*100:.1f}% → {st['local_trail_pct']*100:.1f}% (PnL {pnl:.2f}%)")
        ...
        try:
            ...
            for symbol in SYMBOLS:
                ...
                if target_cb < current_cb:
                    ...
                    if ok:
                        logging.info(f"{symbol} {side} tightened trailing: {current_cb:.1f}% → {target_cb:.1f}% (PnL {pnl:.2f}%)")
        except Exception as e:
            logging.error(f"dynamic_trailing_scheduler error: {e}")

        await asyncio.sleep(1.0)

# ───────────── ROLLING ANALYSIS SCHEDULER (ws candles) ─────────
async def rolling_analysis_scheduler(cli: AsyncClient):
    """Run rolling analysis every 60 seconds using websocket-fed candles."""
    while True:
        try:
            for symbol in SYMBOLS:
                st = state[symbol]
                ...
                if now - st["last_analysis_time"] >= ANALYSIS_INTERVAL_SECONDS:
                    ...
                    signal = await perform_rolling_analysis_ws(symbol, current_price)
                    if signal:
...
# ─── Helper: fetch recent swing levels from ws candles
def get_recent_swing_levels_from_store(symbol: str):
    try:
        limit = max(30, min(1000, CONFIRM_LOOKBACK_MIN))
        candles = list(candle_store[symbol])[-limit:]
    ...
    async with st["order_lock"]:
        ...
        if exited_side == "LONG":
            if await safe_order_execution(cli, open_short(symbol), symbol, "SHORT ENTRY (FLIP on TS fill)"):
                ...
                if CONFIRM_FLIP:
                    recent_high, recent_low = get_recent_swing_levels_from_store(symbol)
...
# ─ NEW: listen for order updates + account updates (positions) to avoid REST
            ...
            async with websockets.connect(url) as ws:
                async for raw in ws:
                    ...
                    if etype == "ORDER_TRADE_UPDATE":
                        ...
                        for p in positions:
                            symbol = p.get("s")
                            ps = p.get("ps")           # LONG/SHORT
                            pa = float(p.get("pa", "0"))
                            if symbol not in state:
                                continue
                            st = state[symbol]
                            if ps == "LONG":
                                st["pos_size_long"] = abs(pa)
                                st["in_long"] = pa != 0.0
                                if pa == 0.0:
                                    st["long_entry_price"] = None
                            elif ps == "SHORT":
                                st["pos_size_short"] = abs(pa)
                                st["in_short"] = pa != 0.0
                                if pa == 0.0:
                                    st["short_entry_price"] = None
        except Exception as e:
...
async def run(cli: AsyncClient):
    ...
    asyncio.create_task(user_stream_listener(cli))      # order + positions via WS
    asyncio.create_task(dynamic_trailing_scheduler(cli)) # dynamic tightening

    # Add trade and 1m kline streams for each symbol (no REST klines)
    trade_streams = [f"{s.lower()}@trade" for s in SYMBOLS]
    kline_streams = [f"{s.lower()}@kline_1m" for s in SYMBOLS]
    all_streams = trade_streams + kline_streams
    url = f"wss://stream.binance.com:9443/stream?streams={'/'.join(all_streams)}"
    ...
    async with websockets.connect(url) as ws:
        async for raw in ws:
            ...
            # 1m klines (closed candles only)
            elif "@kline_1m" in stype:
                k = d.get("k", {})
                sym = k.get("s")
                if sym not in candle_store:
                    continue
                closed = bool(k.get("x"))     # candle closed
                if not closed:
                    continue
                # store closed candle as [openTime, o,h,l,c,v]
                otime = int(k.get("t"))
                o = float(k.get("o")); h = float(k.get("h"))
                l = float(k.get("l")); c = float(k.get("c"))
                v = float(k.get("v"))
                dq = candle_store[sym]
                # ensure chronological push (avoid duplicates)
                if len(dq) == 0 or dq[-1][0] < otime:
                    dq.append([otime, o, h, l, c, v])
                elif dq[-1][0] == otime:
                    dq[-1] = [otime, o, h, l, c, v]
